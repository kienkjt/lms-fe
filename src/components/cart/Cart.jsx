...
const Cart = () => {
    ...
    if (isAuthenticated) {
      cartService.getCart()
        .then(res => {
          ...
          const total = cartItems.reduce((sum, item) => sum + (item.course?.discountPrice || item.course?.price || 0), 0);
  ...
  const handleClear = async () => {
    ...
    } catch { toast.error('Không thể xóa giỏ hàng'); }
  };

  const total = items.reduce((sum, item) => sum + (item.course?.discountPrice || item.course?.price || 0), 0);
  ...
  return (
    <div className="cart-page">
      <div className="container">
        <h1 style={{ marginBottom: 'var(--space-6)' }}>Giỏ hàng của tôi 🛒</h1>
        ...
        {items.length === 0 ? (
          <div className="empty-state" style={{ padding: '80px 0' }}>
            ...
            <Link to={ROUTES.COURSES} className="btn btn-primary">Khám phá khóa học</Link>
          </div>
        ) : (
                ...
                return (
                  <div key={item.courseId || item.id} className="cart-item">
                    ...
                    <div className="cart-item-info">
                      <Link to={`/courses/${course?.slug}`} className="cart-item-title">
                        {course?.title}
                      </Link>
                      <p className="cart-item-instructor">{course?.instructorName}</p>
              ...
              <div className="card">
                <div className="card-body">
                  ...
                  <div className="summary-row summary-total">
                    <span>Tổng cộng:</span>
                    <span style={{ color: 'var(--primary)', fontSize: '22px', fontWeight: 800 }}>
                      {formatPrice(total)}
                    </span>
                  </div>
                  ...
                  <Link to={ROUTES.COURSES} className="btn btn-ghost btn-full" style={{ marginTop: '8px' }}>
                    Tiếp tục mua hàng
                  </Link>
                </div>
              </div>
